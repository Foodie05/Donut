...
class AiService {
  ...
  late final String _effectiveApiKey;
  ...
  AiService(this.apiKey, this.baseUrl, this.modelName, this.modelReplyLength) {
    _effectiveApiKey = apiKey.isNotEmpty ? apiKey : defaultGatewayApiKey;
    _effectiveBaseUrl = baseUrl.isNotEmpty ? baseUrl : defaultGatewayBaseUrl;
    _effectiveModelName = modelName.isNotEmpty
        ? modelName
        : defaultGatewayModelName;

    _client = openai.OpenAIClient(
      config: openai.OpenAIConfig(
        authProvider: openai.ApiKeyProvider(_effectiveApiKey),
    ...
    try {
      ...
      final stream = _client.chat.completions.createStream(
        openai.ChatCompletionCreateRequest(
          ...
          messages: [openai.ChatMessage.user(contentParts)],
  ...
  Stream<String> chatWithPage({
    ...
    final contentParts = <openai.ContentPart>[openai.ContentPart.text(prompt)];

    if (summary != null && summary.isNotEmpty) {
      contentParts.insert(
        0,
        openai.ContentPart.text("Page Summary: $summary\n\n"),
      );
    }
